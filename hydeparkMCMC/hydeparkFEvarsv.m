%% FE-VAR-SV model
% FE modeled as VAR-SV (without MDS assumption, no transformation into etas)
...
for p = [2 5]
    ...
    for d =  1  : length(DATALABELS)
        ...
        modellabel = strcat(modellabel, sprintf('lags%d', p));
        ...
        load(matfile, 'Yforecast', 'Yrealized', 'FE', 'FEobserved', 'Nsurvey', 'dates', 'horizons', 'maxHorizon', 'Ylabel')
        ...
        parfor thisT = Tstart : T % @parfor
            ...
            TID = parid;
            [mcmc_fe, mcmc_maxlambda, mcmc_hVCV] = mcmcsamplerFEVARSV(FEobserved, ...
                FEnanny, thisT, p, horizons, MCMCdraws, Nfedraws, doDiffuseSlopes, rndStreams{TID}, TID == 1); %#ok<PFBNS>
        ...
        %% compute CRPS
        crps  = NaN(T,Nsurvey);
        foodraws = permute(feDraws, [3 1 2]); % should make the following loop quicker
        parfor n = 1 : Nsurvey % TODO check: is this order of loops good for performance?
            for t = 1 : T % could just start at Tstart
                % crps(t,n) = crpsDraws(FE(t,n), feDraws(t,n,:));
                crps(t,n) = crpsDraws(FE(t,n), foodraws(:,t,n));
            end % t
        end % n
        if ~isequal(isnan(crps), isnan(FE) | any(isnan(feDraws),3))
            error('crps has NaN when there is data')
        end
        clear foodraws
        ...
        for n = 1 : Nsurvey
            figure
            hold on
            plotCI(feMean(:,n), squeeze(feTails(:,n,:)), dates)
            xtickdates(dates(evalTstart:end))
            title(sprintf('FE h=%d',n-1));
            wrapcf(sprintf('%sFEtails%d', datalabel, n), wrap)
        end
