...
for d =  1 : length(DATALABELS)
    ...
    modellabel = strcat(modellabel, 'QRTconst');
    ...
    parfor thisT = Tstart : T-1 % @parfor
        TID = parid;
        [mcmc_feSV, mcmc_fe, mcmc_etaSV] = mcmcsamplerConst(eta, etaNanny, RTwindow, thisT, horizons, MCMCdraws, Nfedraws, doZeroSlopes, doDiffuseSlopes, rndStreams{TID}); %#ok<*PFBNS>
    ...
    [mcmc_feSV, mcmc_fe, mcmc_etaSV] = mcmcsamplerConst(eta, etaNanny, RTwindow, thisT, horizons, MCMCdraws, Nfedraws, doZeroSlopes, doDiffuseSlopes, rndStreams{TID});
    feSVdraws(thisT,:,:)             = mcmc_feSV(end,:,:);
    etaSVdraws(thisT,:,:)            = mcmc_etaSV(end,:,:);
    feDraws(thisT,:,:)               = mcmc_fe;
    feVol(thisT,:)                   = std(mcmc_fe,0,2);
