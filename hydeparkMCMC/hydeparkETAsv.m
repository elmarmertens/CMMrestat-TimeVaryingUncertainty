...
%% loop over evaluation windows
for Tstart = [60 80]

    RTwindow   = Tstart;
    evalTstart = Tstart;
    % note: this code re-estimates the MCMC runs for both choices of evalTstart 
    % (alternative code could also estimate MCMC runs only for smallest choice of evalTstart and reuse those results for higher choices of evalTstart)
    ...
    for d =  1 : length(DATALABELS)
        ...
        parfor thisT = Tstart : T - 1 % @parfor
            TID = parid;
            [mcmc_feSV, mcmc_fe, mcmc_etaSV, mcmc_hVCV] = mcmcsamplerSV(eta, etaNanny, thisT, horizons, MCMCdraws, Nfedraws, doZeroSlopes, doDiffuseSlopes, rndStreams{TID}, TID == 1); %#ok<PFBNS>
        ...
        [mcmc_feSV, mcmc_fe, mcmc_etaSV, mcmc_hVCV] = mcmcsamplerSV(eta, etaNanny, thisT, horizons, MCMCdraws, Nfedraws, doZeroSlopes, doDiffuseSlopes, rndStreams{TID}, true);
        feSVdraws(thisT,:,:)             = mcmc_feSV(end,:,:);
        etaSVdraws(thisT,:,:)            = mcmc_etaSV(end,:,:);
        feDraws(thisT,:,:)               = mcmc_fe;
        feVol(thisT,:)                   = std(mcmc_fe,0,2);
