%%  ETA-SV model but using AR1 for SV (rather than RW as in baseline)
% uses MDS assumption
...
for d =  1 : length(DATALABELS)
    ...
    modellabel = strcat(modellabel,'QRTdensitySVAR1');
    ...
    hrhodraws         = NaN(Nsurvey,MCMCdraws,T);
    hbardraws         = NaN(Nsurvey,MCMCdraws,T);
    
    
    parfor thisT = Tstart : T - 1 % @parfor
        TID = parid;
        [mcmc_feSV, mcmc_fe, mcmc_etaSV, mcmc_hVCV, mcmc_RHO, mcmc_HBAR] = mcmcsamplerSVar1(eta, etaNanny, thisT, horizons, MCMCdraws, Nfedraws, doZeroSlopes, doDiffuseSlopes, rndStreams{TID}, false); %#ok<PFBNS>
    ...
    [mcmc_feSV, mcmc_fe, mcmc_etaSV, mcmc_hVCV, mcmc_RHO, mcmc_HBAR] = mcmcsamplerSVar1(eta, etaNanny, thisT, horizons, MCMCdraws, Nfedraws, doZeroSlopes, doDiffuseSlopes, rndStreams{TID}, true);
    feSVdraws(thisT,:,:)             = mcmc_feSV(end,:,:);
    etaSVdraws(thisT,:,:)            = mcmc_etaSV(end,:,:);
    feDraws(thisT,:,:)               = mcmc_fe;
    feVol(thisT,:)                   = std(mcmc_fe,0,2);
    ...
    %% plot rho
    tails = prctile(hrhodraws, [5 95], 2);
    figure
    for n = 1 : Nsurvey
        subplot(Nsurvey, 1, n)
        hold on
        plot(dates, squeeze(mean(hrhodraws(n,:,:), 2)), 'b-', 'linewidth', 2)
        plot(dates, permute(tails(n,:,:), [3 2 1]), 'b-')
        ylim([-.5 1])
        xtickdates(dates([Tstart T]))
    end
    wrapcf(sprintf('hrho%s', modellabel), wrap)
    
    %% plot hbar
    SVbardraws = exp(0.5 * hbardraws);
    tails = prctile(SVbardraws, [5 95], 2);
    figure
    for n = 1 : Nsurvey
        subplot(Nsurvey, 1, n)
        hold on
        plot(dates, squeeze(mean(SVbardraws(n,:,:), 2)), 'b-', 'linewidth', 2)
        plot(dates, permute(tails(n,:,:), [3 2 1]), 'b-')
        ylim([0 max(ylim)])
        xtickdates(dates([Tstart T]))
    end
    wrapcf(sprintf('SVbar%s', modellabel), wrap)
